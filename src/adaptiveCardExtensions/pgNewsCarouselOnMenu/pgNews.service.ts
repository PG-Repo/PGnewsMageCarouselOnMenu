...
export const fetchSitePageLibrary = async (spContext: AdaptiveCardExtensionContext): Promise<IListItem[]> => {
  ...
  // spContext.pageContext.site.absoluteUrl+JSON.parse(listItem.PGImage).serverRelativeUrl
  // spContext.pageContext.site.absoluteUrl+listItem.PGImage.match('"serverRelativeUrl":(.*),"id"')[1].replace(/["]/g, '')
